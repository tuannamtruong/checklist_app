 ...
 *
 * The stock AssetsPathHandler guesses the type from the file extension and gets
 * two of ours wrong. A module script served as anything but a JavaScript MIME
 * type is refused outright — strict checking, no fallback — so the app would
 * fail to boot with nothing but a console message to show for it, and the PWA
 * manifest is ignored unless it arrives as its own type.
 */
